...
public class ArrivalCurveEditorPanel extends JPanel {
    ...
    private static final int PAD_B = 85;
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.drawString("Arrival Curve Editor (" + mode + ") — drag lines: peak / σ / clamp", plotL, 14);
        ...
        // Handles
        drawHandleLine(g2, plotT, plotB, config.getPeakMinutesBeforeDeparture(),
                "peak=" + config.getPeakMinutesBeforeDeparture(), new Color(220, 80, 80));

        drawHandleLine(g2, plotT, plotB, config.getPeakMinutesBeforeDeparture() + config.getLeftSigmaMinutes(),
                "Lσ=" + config.getLeftSigmaMinutes(), new Color(80, 170, 90));

        drawHandleLine(g2, plotT, plotB, config.getPeakMinutesBeforeDeparture() - config.getRightSigmaMinutes(),
                "Rσ=" + config.getRightSigmaMinutes(), new Color(80, 170, 90));
        ...
        // Bottom labels (ticks + axis label + summary lines)
        g2.setColor(Color.DARK_GRAY);
        drawBottomLabels(g2, plotL, plotR, plotB);
    ...
    // New: tick labels centered + adaptive tick density
    private void drawXTicks(Graphics2D g2, int plotL, int plotR, int plotB, int yText) {
        int plotW = plotR - plotL;

        // If the panel is narrow, draw fewer ticks so labels never overlap
        int[] ticks = (plotW < 520)
                ? new int[]{240, 180, 120, 60, 20}
                : new int[]{240, 210, 180, 150, 120, 90, 60, 30, 20};
        ...
        for (int t : ticks) {
            ...
            // center number under the tick
            g2.drawString(s, x - (sw / 2), yText);
        }
    }

    // New: bottom spacing so nothing overlaps / jumbles
    private void drawBottomLabels(Graphics2D g2, int plotL, int plotR, int plotB) {
        FontMetrics fm = g2.getFontMetrics();

        // Place bottom content on separate lines (prevents jumbling)
        int yTicks    = plotB + fm.getAscent() + 6;
        int yAxis     = yTicks + fm.getHeight() + 2;
        int ySummary1 = yAxis  + fm.getHeight() + 2;
        int ySummary2 = ySummary1 + fm.getHeight() + 2;

        // tick marks + numbers
        drawXTicks(g2, plotL, plotR, plotB, yTicks);

        // axis caption line
        g2.drawString("minutes before departure (240 -> 20)", plotL, yAxis);

        // summary split into two lines (so it always stays readable)
        String line1 = "peak=" + config.getPeakMinutesBeforeDeparture()
                + "   Lσ=" + config.getLeftSigmaMinutes()
                + "   Rσ=" + config.getRightSigmaMinutes();

        String line2 = "windowStart=" + config.getWindowStartMinutesBeforeDeparture()
                + "   close=" + config.getBoardingCloseMinutesBeforeDeparture()
                + "   clamp=" + (config.isLateClampEnabled()
                ? config.getLateClampMinutesBeforeDeparture()
                : "OFF");

        g2.drawString(line1, plotL, ySummary1);
        g2.drawString(line2, plotL, ySummary2);
    }
    ...
    private int[] buildPreviewCounts() {
        ...
        if (config.isLegacyMode()) {
            ...
            // We approximate the legacy shape without needing Flight
